    ...
    public function map()
    {
        Route::namespace($this->namespace)->group(function (Router $router) {
                ...
                $router->middleware('public')->group(function (Router $router) use ($page) {
                    ...
                    foreach (locales() as $lang) {
                        if ($page->translate('status', $lang) && $uri = $page->uri($lang)) {
                            ...
                            $router->get($uri.'/invoice/{number}', $options + ['uses' => 'PublicController@invoice'])->name($lang.'::subscriptions-invoice');
                            $router->get($uri.'/invoice/{number}/download', $options + ['uses' => 'PublicController@downloadInvoice'])->name($lang.'::subscriptions-download-invoice');
                        }
                    }
                });
            }
            ...
            $router->middleware('admin')->prefix('admin')->group(function (Router $router) {
                ...
                $router->post('subscriptions/{subscription}/cancel', 'AdminController@cancel')->name('admin::cancel-subscription')->middleware('can:update-subscription');
                $router->post('subscriptions/{subscription}/resume', 'AdminController@resume')->name('admin::resume-subscription')->middleware('can:update-subscription');
            });
